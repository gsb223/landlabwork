...
# This file is to simulate increasing level rainfall on the Alexauken Creek topography created with fastscape landlab component. 
...
# SETTING UP COMPONENTS AND PARAMETERS
...
# SETTING UP OLD TOPO -- for finding the change in topo
mg.add_zeros('node', 'old__topo')  # new field
mg.add_zeros('node', 'change__topo')  # new field
...
# FOR INCREASING RAINFALL
start_int = 0.081026 #3.19in
five_yr_int = 0.117094 #4.61in
five_below_rt = (five_yr_int-start_int)/5
ten_yr_int = 0.137922 #5.43in
ten_below_rt = (ten_yr_int-five_below_rt)/5
twentyfive_yr_int = 0.16891#6.65in
twentyfive_below_rt = (twentyfive_yr_int-ten_yr_int)/15
# after 5 years it get to five year sate and after 10 years it is at 10 year rate and etc. Based on point precipitaiton values 
...
# VARIABLES FOR NAMEING PNGS
title_yr = 1
num_topo = 0
num_sed = 0
# RUN THE MODEL
while model_elapsed_years < model_total_years:  # over the years
    # determine the rainfall amount based on what year it is 
    # every year adding to the rainfall 
    if model_elapsed_years <5 and not model_elapsed_years == 0 : #yrs 0-4
        stm_avg_int += five_below_rt
    elif model_elapsed_years > 4 and model_elapsed_years < 10:
        stm_avg_int += ten_below_rt
    elif model_elapsed_years >9 and model_elapsed_years < 25:
        stm_avg_int += twentyfive_below_rt
    ...
    for x in range(1, 5):  # this is to loops tho the different seasons and change the interstorm durations
        ...
        fr.run_one_step()
        # this is run every season -- more stable timestep
    ...
    if model_elapsed_years > start_year:  # now you can start saving all of the data
        ...
        figelev = plt.figure()  # Instantiate figure as empty plot
        plot = plt.subplot()  # Instantiate subplot as empty plot
        imshow_grid(mg, 'sediment__flux', vmin=-2E-10, vmax=2E-10, var_name='Sediment Flux ($m^3$)', grid_units=('m', 'm'), cmap='terrain', color_for_background=None)
     #  this is essential to having clean axis labels
        ...
        fname = "exp/vids/incr_alex_sed_video/alex_incr_sed_flux_" + str(num_sed) + "_"+".png"
        figelev.savefig(fname, dpi='figure', format=None)
        plt.close(figelev)
        num_sed += 1

        # TOPOGRAPHIC CHANGE IMAGE
        figelev = plt.figure()  # Instantiate figure as empty plot
        plot = plt.subplot()  # Instantiate subplot as empty plot
        imshow_grid(mg, 'change__topo', vmin=-.3, vmax=.3, var_name='Elevation Change',           grid_units=('m', 'm'), cmap='terrain', color_for_background=None)
        plot.set_xticklabels([0, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000, 9000])
        plot.set_yticklabels([0, 1000, 2000, 3000, 4000, 5000, 6000, 7000, 8000])
        fname="exp/vids/INCR_alex_topo_video/alex_INCR_change_topo_" +str(num_topo) + "_yrs"+".png"
        figelev.savefig(fname, dpi='figure', format=None)
        plt.close(figelev)
        num_topo += 1 # for the title
        title_yr += 1  # for the title

    model_elapsed_years += 1

# EXPORT THE MODEL GRID TO BE IMPORTED FOR THE HURRICANE
grid_fields_to_save=["topographic__elesvation"]
fname="ALEX_INCR_int_topo.txt"  # name and location to save too
