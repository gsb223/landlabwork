...
# This file is to simulate increasing level rainfall on the synthetic topography created with fastscape landlab component. 
...
title_yr = 1 # for changing title names 

#print statements in the loop are so you can see in real time where in the model you are at
stm_avg_int = start_int #to get the initial rain amount
...
while model_elapsed_years < model_total_years:#over the years
    ...
    if model_elapsed_years > start_year: # now you can start saving all of the data
        ...
        # PLOT THE TOPOGRAPHY CHANGE AND SAVE
        figelev = plt.figure()#Instantiate figure as empty plot
        plot = plt.subplot()#Instantiate subplot as empty plot
        ...
        fname = "exp/sample_incr/sample_incr_change_topo"+ str(model_elapsed_years) +"_yrs"+".png"# location and name to save png  
        ...
        plot = plt.subplot()#Instantiate subplot as empty plot
        imshow_grid(mg,'topographic__elevation', var_name = 'Elevation', var_units=r'm', grid_units=('m', 'm'), cmap='terrain',color_for_background=None)
        fname = "exp/sample_incr/sample_incr_topo"+ str(model_elapsed_years) +"_yrs"+".png" # location and name to save png 
        figelev.savefig(fname, dpi='figure', format=None)
        plt.close(figelev)
        ...
        fname = "exp/sample_incr/sample_incr_sed_flux"+ str(model_elapsed_years) +"_yrs"+".png" # location and name to save png 
        figelev.savefig(fname, dpi='figure', format=None)
        plt.close(figelev)
        title_yr+=1 # for the title 

    model_elapsed_years +=1 # go on to the next year
